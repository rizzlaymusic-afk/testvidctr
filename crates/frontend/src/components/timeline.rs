...
pub fn Timeline() -> impl IntoView {
    ...
    // Keyboard handlers for trim handles: Arrow keys move by 100ms, PageUp/PageDown by 1s, Home/End to bounds
    let on_start_handle_key = {
        ...
        move |ev: KeyboardEvent| {
            ...
            match key.as_str() {
                ...
                "PageDown" => new_start = (new_start - 1000.0).max(0.0),
                "PageUp" => new_start = (new_start + 1000.0).min(state.trim_end_ms.get()),
                "Home" => new_start = 0.0,
                "End" => new_start = state.trim_end_ms.get(),
                _ => return,
            }
    ...
    let on_end_handle_key = {
        ...
        move |ev: KeyboardEvent| {
            ...
            match key.as_str() {
                ...
                "PageDown" => new_end = (new_end - 1000.0).max(state.trim_start_ms.get()),
                "PageUp" => new_end = (new_end + 1000.0).min(state.duration_ms.get()),
                "Home" => new_end = state.trim_start_ms.get(),
                "End" => new_end = state.duration_ms.get(),
                _ => return,
            }
    ...
    view! {
        <div class="timeline-container">
            ...
            <div class="timeline-track" on:click=on_timeline_click aria-label="Timeline, click to seek" tabindex="0">
                ...
                <div class="trim-handle start" tabindex="0" role="slider" aria-label="Trim start" aria-orientation="horizontal" aria-controls="player" aria-valuemin=move || "0" aria-valuemax=move || state.duration_ms.get().to_string() aria-valuenow=move || state.trim_start_ms.get().to_string() aria-valuetext=move || format_timecode(state.trim_start_ms.get()) on:keydown=on_start_handle_key style=move || format!("left: {}%", trim_start_pct()) />
                <div class="trim-handle end" tabindex="0" role="slider" aria-label="Trim end" aria-orientation="horizontal" aria-controls="player" aria-valuemin=move || state.trim_start_ms.get().to_string() aria-valuemax=move || state.duration_ms.get().to_string() aria-valuenow=move || state.trim_end_ms.get().to_string() aria-valuetext=move || format_timecode(state.trim_end_ms.get()) on:keydown=on_end_handle_key style=move || format!("left: {}%", trim_end_pct()) />
